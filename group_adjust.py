...
def group_adjust(vals, groups, weights):
    ...
    valslen = len(vals)
    if (weightslen != groupslen):
        raise ValueError('length of weights {} isnt length of groups {}'.format(weightslen, groupslen))

    for itr in range(0, groupslen):
        group = groups[itr]
        grouplen = len(group)
        if (valslen != grouplen):
            raise ValueError('length of vals {} isnt length of group {}'.format(valslen, group))
    ...
    tableToConcat = df0
    for itr in range(0, groupslen):
        pt = pd.pivot_table(df, index=[groupnames[itr]], values=['quant'], aggfunc=[np.mean])
        pt = pt.reset_index()
        pt.columns = [groupnames[itr], groupnames[itr] + 'mean']
        tableToConcat = tableToConcat.merge(pt, on=groupnames[itr])

    demeanlist = []
    for index, row in tableToConcat.iterrows():
        weightedmean = 0
        for itr in range(0, groupslen):
            weightedmean += (weights[itr] * row[groupmeannames[itr]])
        demeanlist += [row['quant'] - weightedmean]
